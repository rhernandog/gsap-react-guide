...
class MyComponent extends Component {
  ...
  render(){
    return <div ref={el => this.myElement = el}>
    </div>;
  }
}
```
...
render(){
  return <div>
    <ul>
      {elementsArray.map((element, index) => <li key={element.id}
        ref={el => this.myTween.to(el, 0.5, {y: 0, autoAlpha: 1, delay: (0.1 * index)})}
      >
        {element.name}
      </li>)}
    </ul>
...
```
Now this looks a bit more complex but the principle remains. Basically we're using the same pattern to access each DOM node, the only difference is that we're not storing them in the class instance, we just need a temporal reference that's stored by the Timeline's child instances (created by the shorthand methods) and we also use the index provided by `array.map()` to create the stagger effect.

#### INSERT MULTIPLE ELEMENTS EXAMPLE
...
But we won't always get all the elements in an array, sometimes we might need to create a complex animation using different elements. Just like in the first example we store a reference in the constructor and create our timeline in the `componentDidMount` method:

#### INSERT TIMELINE SEQUENCE SAMPLE
...
#### INSERT CONTROL THROUGH STATE SAMPLE
...
#### INSERT SIMPLE TRANSITION SAMPLE
...
#### INSERT TRANSITION GROUP SAMPLE
...
#### INSERT REACT ROUTE ANIMATION SAMPLE
